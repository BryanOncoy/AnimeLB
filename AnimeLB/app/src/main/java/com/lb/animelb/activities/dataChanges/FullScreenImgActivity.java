...
public class FullScreenImgActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        currentUser.setProfilePicFull(imageFullScreen);
        currentUser.updateProfilePicFull();

        btPhoto.setOnClickListener(v -> askCameraPermission());

        btPickImage.setOnClickListener(v -> {
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == CAMERA_REQUEST_CODE) {
            if (resultCode == Activity.RESULT_OK) {
                currentUser.uri = Uri.fromFile(new File(currentPhotoPath));
                updatePics();
            }
        }
        if (requestCode == GALLERY_REQUEST_CODE && resultCode == RESULT_OK && data != null) {
            currentUser.uri = data.getData();
            updatePics();
        }
    }
